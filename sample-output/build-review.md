...
## Bottom line
This is not safe to run. Three Critical issues (SQL injection, a broken access
control that also injects, and a hardcoded key) and two High issues (an open
admin route that also leaks passwords) have to be fixed first. Work the list
worst-first, re-run this review, and the runtime and human items above still
need you to close them out yourself.
